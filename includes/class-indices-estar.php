...
class Indices_Estar {
	...
	public function init(): void {
		...
		// Detectar si algún post del query usa el shortcode para encolar assets sólo cuando sea necesario
		add_filter( 'the_posts', static function ( array $posts ): array {
			if ( empty( $posts ) ) return $posts;
			foreach ( $posts as $p ) {
				if ( $p instanceof WP_Post && has_shortcode( $p->post_content, 'indices_estar' ) ) {
					self::$should_enqueue_public = true;
					break;
				}
			}
			return $posts;
	...
	public function public_assets(): void {
		...
		wp_enqueue_style(
			'indices-estar-public',
			INDICES_ESTAR_URL . 'public/assets/public.css',
			[],
			INDICES_ESTAR_VERSION
		);
		wp_enqueue_script(
			'indices-estar-public',
			INDICES_ESTAR_URL . 'public/assets/public.js',
			[],
			INDICES_ESTAR_VERSION,
			true
		);
